...
public class VideoDataMediator {
    ...
    public Collection<String> getLikedByList(Video v)
    {
        try {
            return mVideoServiceProxy.getUsersWhoLikedVideo(v.getId());
        } catch(Exception e) {
            return null;
        }
    }
    ...
    {
        try {
            mVideoServiceProxy.likeVideo(videoID, ""); // empty string so retrofit doesn't complain
            return true;
        }
        catch(Exception e)
        {
    ...
    {
        mVideoServiceProxy.unlikeVideo(videoID, ""); //empty string so retrofit doesn't complain
    }
}
